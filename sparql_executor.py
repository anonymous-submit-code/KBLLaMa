...
def execute_binary(relation: str) -> List[Tuple[str, str]]:
    ...
    rtn = []
    for result in results['results']['bindings']:
        rtn.append((result['x0']['value'].replace('http://rdf.freebase.com/ns/', ''), result['x1']['value'].replace('http://rdf.freebase.com/ns/', '')))
...
def get_friendly_desc(entity: str) -> str:
    query = ("""
    PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    PREFIX : <http://rdf.freebase.com/ns/> 
    ...
    SELECT DISTINCT ?x0  WHERE {
    """
             ':' + entity + ' :common.topic.description ?x0 . '
                            """
    }
    }
    """)
...
def get_relation_prune_entity_all(relation: str, mode):
    entities = set()
    if mode == 'sub':
        query2 = ("""
            PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    ...
    try:
        ...
        exit(0)
    for result in results['results']['bindings']:
        entities.add(result['value']['value'].replace('http://rdf.freebase.com/ns/', ''))

    return entities

def get_relation_prune_entity(relation: str, mode, class_type:str):
    entities = set()
    if mode == 'sub':
        query2 = ("""
            PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
            ...
            SELECT DISTINCT ?x0  WHERE {
            """
                   '?x0 ' + ':' + relation + ' ?x1. \n ?x1 :type.object.type :' + class_type + '.'
                                 """
        FILTER regex(?x0, "http://rdf.freebase.com/ns/")
        }
        }
        ...
        query2 = ("""
            ...
            SELECT DISTINCT ?x1  WHERE {
            """
                   '?x0 ' + ':' + relation + ' ?x1. \n ?x0 :type.object.type :' + class_type + '.'
                                 """
        FILTER regex(?x1, "http://rdf.freebase.com/ns/")
        }
        }
